...
import { SearchOutlined, CloudUploadOutlined, PaperClipOutlined } from '@ant-design/icons-vue';
...
async function handleSendChat() {
  ...
  if (userMessage) {
    chatContentStore.addUserMessage(userMessage)
...
async function sendChatWithStreaming(userMessage: string) {
  ...
  console.log('chatHistoryStore.activeChatHistoryItem:', chatHistoryStore.activeChatHistoryItem)
...
const toggleFunction = (functionName: string) => {
  ...
  if (selectedFunctions.value.has(functionName)) {
    ...
    const promptText = functionPromptMap[functionName as keyof typeof functionPromptMap]
  ...
  console.log('Current prompt text to be added to message:', functionPrompts.value.join(' '))
...
watch(functionPrompts, (newPrompts, oldPrompts) => {
...
watch(selectedFunctions, (newSelected, oldSelected) => {
    ...
    >
      <Flex wrap="wrap" :gap="12" align="center">
        <Button
          ...
          type="text"
          size="small"
          :class="['header-action-btn', { 'selected': selectedFunctions.has('translate') }]"
        >
          翻译
        </Button>
        <Button
          ...
          type="text"
          size="small"
          :class="['header-action-btn', { 'selected': selectedFunctions.has('mindMap') }]"
        >
          思维导图
        </Button>
        <Button
          ...
          type="text"
          size="small"
          :class="['header-action-btn', { 'selected': selectedFunctions.has('email') }]"
        >
          邮件
        </Button>
      </Flex>
...
.chat-frame-input :deep(.ant-sender-input:focus-within) {
  ...
  box-shadow: 0 0 0 3px rgba(124, 58, 237, 0.1), 0 2px 8px rgba(124, 58, 237, 0.15) !important; /* 聚焦阴影 */
}
