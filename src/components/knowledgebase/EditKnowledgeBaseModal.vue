...
defineOptions({ name: 'EditKnowledgeBaseModal' })
...
const handleUpdate = async () => {
  try {
    ...
    await knowledgeBaseStore.updateKnowledgeBase(updateData)
    
    message.success('知识库更新成功')
    emit('knowledge-base-updated')
...
const loadKnowledgeBaseData = () => {
  if (props.knowledgeBase) {
    ...
    formData.icon = props.knowledgeBase.icon || 'library_books'
    formData.color_variant = props.knowledgeBase.color_variant || 'purple'
  }
}

// Watch for modal visibility and knowledge base changes
watch(() => props.visible, (newVisible) => {
  if (newVisible && props.knowledgeBase) {
    loadKnowledgeBaseData()
  }
})

watch(() => props.knowledgeBase, (newKnowledgeBase) => {
  if (newKnowledgeBase && props.visible) {
    loadKnowledgeBaseData()
  }
})
</script>
...
<style scoped>
.edit-kb-modal :deep(.ant-modal-header) {
  background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);
  border-bottom: 1px solid #e2e8f0;
}

.edit-kb-modal :deep(.ant-modal-title) {
  color: #1e293b;
  font-weight: 600;
}
